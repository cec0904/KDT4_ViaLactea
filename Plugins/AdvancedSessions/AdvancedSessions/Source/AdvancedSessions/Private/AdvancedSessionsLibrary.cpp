...
bool UAdvancedSessionsLibrary::StartListenServer(UObject* WorldContextObject, int32 Port)
{
	UWorld* const World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull);

	// 현재 독립 실행 상태(NM_Standalone)일 때만 리슨 서버를 가동함
	if (World && World->GetNetMode() == NM_Standalone)
	{
		...
		FString Error;
		// 핵심: ServerTravel 전에 엔진이 'Listen' 상태를 강제로 인식하게 함
		if (!World->Listen(ListenURL))
		{
			UE_LOG(AdvancedSessionsLog, Error, TEXT("리슨 서버 시작 실패: %s"), *Error);
			return false;
		}

		UE_LOG(AdvancedSessionsLog, Log, TEXT("리슨 서버가 포트 %d에서 정상 시작되었습니다."), Port);
		return true;
	}
	return false;
}